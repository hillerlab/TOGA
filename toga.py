...
from modules.common import log_and_print
...
class Toga:
    ...
    def __init__(self, args):
        ...
        log_and_print("#### Initiating TOGA class ####")
        self.para = args.para
        ...
        log_and_print(f"Saving output to {self.wd}")
        log_and_print(f"Arguments stored in {self.toga_args_file}")


    def __setup_logging(self):
        # Create a logger with a specific name
        self.logger = logging.getLogger('toga')
    ...
    def __check_2bit_file(self, two_bit_file, chroms_sizes, chrom_file):
        ...
        try:  # try to catch EOFError: if 2bitreader cannot read file
            ...
            log_and_print(f"Found {len(twobit_sequences)} sequences in {two_bit_file}")
            ...
            log_and_print(f"Error! Twobitreader cannot open {two_bit_file}")
            self.die("Abort")
    ...
    def __check_isoforms_file(self, t_in_bed):
        ...
        if not self.isoforms_arg:
            log_and_print("Continue without isoforms file: not provided")
        ...
        log_and_print(f"Writing isoforms data for {len(t_in_both)} transcripts.")
    ...
    def die(self, msg, rc=1):
        ...
        self.__log_blank()
        log_and_print(msg)
        log_and_print(f"Program finished with exit code {rc}\n")
    ...
    def __check_dependencies(self):
        ...
        if c_not_compiled:
            log_and_print("Warning! C code is not compiled, trying to compile...")
        ...
        try:
            ...
            log_and_print("Warning! Some of the required packages are not installed.")
    ...
    def __call_proc(self, cmd, extra_msg=None):
        ...
        log_and_print(f"Calling cmd:\n{cmd}\n")
        rc = subprocess.call(cmd, shell=True)
        if rc != 0:
            log_and_print(extra_msg) if extra_msg else None
            self.die(f"Error! Process:\n{cmd}\ndied! Abort.")
        log_and_print("Command finished with exit code 0.")
    ...
    def run(self):
        ...
        if not self.cesar_ok_merged:
            ...
            log_and_print(cesar_not_ok_message)
    ...
    def __make_indexed_chain(self):
        ...
        # make *.bb file
        log_and_print("Started chain indexing...")
        chain_bst_index(
    ...
    def __make_indexed_bed(self):
        ...
        log_and_print("Started bed file indexing...")
    ...
    def __extract_chain_features(self):
        ...
        log_and_print(f"Extracting chain features, NF project name: {project_name}")
        log_and_print(f"Project path: {project_path}")
        ...
        if self.para:  # run jobs with para, skip nextflow
            ...
            log_and_print(f"Calling parallel process with:\n{cmd}")
            rc = subprocess.call(cmd, shell=True)
    ...
    def __merge_chains_output(self):
        ...
        log_and_print("Merging chain output...")
        merge_chains_output(
    ...
    def __classify_chains(self):
        ...
        log_and_print("Classifying chains")
    ...
    def __get_proc_pseudogenes_track(self):
        ...
        log_and_print("Creating processed pseudogenes track.")
    ...
    def __precompute_data_for_opt_cesar(self):
        ...
        log_and_print("Computing memory requirements for optimized CESAR")
        ...
        project_path = os.path.join(self.nextflow_dir, project_name)
        log_and_print(f"Precomputing cesar regions, NF project name: {project_name}")
        log_and_print(f"Project path: {project_path}")

        if self.para:  # run jobs with para, skip nextflow
            cmd = f'para make {project_name} {precomp_mem_joblist} -q="shortmed"'
            log_and_print(f"Executing parallel step with {cmd}")
            rc = subprocess.call(cmd, shell=True)
        else:  # calling jobs with nextflow
        ...
        if self.output_opt_cesar_regions:
            ...
            log_and_print(f"Bed file containing precomputed regions is saved to: {save_to}")
            sys.exit(0)
    ...
    def __split_cesar_jobs(self):
        ...
        if self.fragmented_genome:
            log_and_print("Detecting fragmented transcripts")
            ...
            for k, v in gene_fragments.items():
                ...
                f.write(f"{k}\t{v_str}\n")
            f.close()
            log_and_print(f"Detected {len(gene_fragments.keys())} fragmented transcripts")
            log_and_print(f"Fragments data saved to {fragm_dict_file}")
        else:
            # no fragment file: ok
            log_and_print("Skip fragmented genes detection")
        ...
        while True:  # Run until all jobs are done (or crashed)
            ...
            for p in processes:
                ...
                if running:
                    all_done = False
            if all_done:
                log_and_print("### CESAR jobs done ###")
                break
            else:
                log_and_print(f"Status check iteration {iter_num} waiting {ITER_DURATION * iter_num} seconds.")
                time.sleep(ITER_DURATION)
                iter_num += 1
        if not self.keep_nf_logs:
            # remove nextflow intermediate files
            log_and_print("Removing nextflow temp files")
    ...
    def __run_cesar_jobs(self):
        ...
        log_and_print(f"Pushing {len(buckets)} CESAR job lists")
        ...
        for b in buckets:
            ...
            if b != 0:  # extract jobs related to this bucket (if it's not 0)
                ...
                if len(bucket_tasks) == 0:
                    log_and_print(f"There are no jobs in the {b}Gb bucket")
                    continue
        ...
        for p_name in project_names:
            ...
            log_and_print(f"para time output for {p_name}:")
            log_and_print(stdout)
    ...
    def __check_cesar_completeness(self):
        ...
        if len(rejected_logs_filenames) == 0:
            # nothing crashed
            log_and_print("No CESAR jobs crashed")
            return
        ...
        log_and_print(f"{crashed_jobs_num} CESAR jobs crashed, trying to run again...")
        ...
        for bucket, jobs in bucket_to_jobs.items():
            log_and_print(f"Pushing {len(jobs)} jobs into {bucket} GB queue")
        ...
        if len(crashed_twice) == 0:
            log_and_print("All CESAR jobs re-ran succesfully!")
            return
    ...
    def __merge_cesar_output(self):
        ...
        log_and_print("Merging CESAR output to make fasta and bed files.")
        ...
        if len(all_ok) == 0:
            ...
            log_and_print("CESAR results merged")
    ...
    def __gene_loss_summary(self):
        ...
        log_and_print("Calling gene loss summary")
    ...
    def __orthology_type_map(self):
        ...
        log_and_print("Calling orthology types mapping step...")
    ...
    def __check_crashed_cesar_jobs(self):
        ...
        log_and_print(f"{len(self.crashed_cesar_jobs)} CESAR wrapper commands failed")
