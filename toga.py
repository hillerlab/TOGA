...
from modules.common import print_and_log
...
class Toga:
    ...
    def __init__(self, args):
        ...
        print_and_log(self.log_file, "#### Initiating TOGA class ####")
        self.para = args.para
        ...
        print_and_log(self.log_file, f"Saving output to {self.wd}")
        print_and_log(self.log_file, f"Arguments stored in {self.toga_args_file}")
    ...
    def __check_2bit_file(self, two_bit_file, chroms_sizes, chrom_file):
        ...
        try:  # try to catch EOFError: if 2bitreader cannot read file
            ...
            print_and_log(self.log_file, f"Found {len(twobit_sequences)} sequences in {two_bit_file}")
            ...
            print_and_log(self.log_file, str(err))
            print_and_log(self.log_file, f"Error! Twobitreader cannot open {two_bit_file}")
            self.die("Abort")
    ...
    def __check_isoforms_file(self, t_in_bed):
        ...
        if not self.isoforms_arg:
            print_and_log(self.log_file, "Continue without isoforms file: not provided")
        ...
        print_and_log(self.log_file, f"Writing isoforms data for {len(t_in_both)} transcripts.")
    ...
    def die(self, msg, rc=1):
        ...
        self.__log_blank()
        print_and_log(self.log_file, msg)
        print_and_log(self.log_file, f"Program finished with exit code {rc}\n")
    ...
    def __check_dependencies(self):
        ...
        if c_not_compiled:
            print_and_log(self.log_file, "Warning! C code is not compiled, trying to compile...")
        ...
        try:
            ...
            print_and_log(self.log_file, "Warning! Some of the required packages are not installed.")
    ...
    def __call_proc(self, cmd, extra_msg=None):
        ...
        print_and_log(self.log_file, f"Calling cmd:\n{cmd}\n")
        rc = subprocess.call(cmd, shell=True)
        if rc != 0:
            print_and_log(self.log_file, extra_msg) if extra_msg else None
            self.die(f"Error! Process:\n{cmd}\ndied! Abort.")
        print_and_log(self.log_file, "Command finished with exit code 0.")
    ...
    def run(self):
        ...
        self.__mark_start()
        print_and_log(self.log_file, "#### STEP 0: making chain and bed file indexes\n")
        ...
        print_and_log(self.log_file, "#### STEP 1: Generate extract chain features jobs\n")
        self.__split_chain_jobs()
        self.__time_mark("Split chain jobs")

        # 2) extract chain features: parallel process
        print_and_log(self.log_file, "#### STEP 2: Extract chain features: parallel step\n")
        self.__extract_chain_features()
        self.__time_mark("Chain jobs done")

        # 3) create chain features dataset
        print_and_log(self.log_file, "#### STEP 3: Merge step 2 output\n")
        ...
        print_and_log(self.log_file, "#### STEP 4: Classify chains using gradient boosting model\n")
        self.__classify_chains()
        self.__time_mark("Chains classified")

        # 5) create cluster jobs for CESAR2.0
        print_and_log(self.log_file, "#### STEP 5: Generate CESAR jobs")
        ...
        print_and_log(self.log_file, "# STEP 6: Create processed pseudogenes track\n")
        self.__get_proc_pseudogenes_track()

        # 7) call CESAR jobs: parallel step
        print_and_log(self.log_file, "#### STEP 7: Execute CESAR jobs: parallel step\n")
        ...
        print_and_log(self.log_file, "#### STEP 8: Merge STEP 7 output\n")
        ...
        print_and_log(self.log_file, "#### STEP 9: Gene loss pipeline classification\n")
        ...
        print_and_log(self.log_file, "#### STEP 10: Create orthology relationships table\n")
        self.__orthology_type_map()

        # 11) merge logs containing information about skipped genes,transcripts, etc.
        print_and_log(self.log_file, "#### STEP 11: Cleanup: merge parallel steps output files")
        ...
        if not self.cesar_ok_merged:
            ...
            print_and_log(self.log_file, cesar_not_ok_message)
    ...
    def __make_indexed_chain(self):
        ...
        # make *.bb file
        print_and_log(self.log_file, "Started chain indexing...")
        chain_bst_index(
    ...
    def __make_indexed_bed(self):
        ...
        print_and_log(self.log_file, "Started bed file indexing...")
    ...
    def __extract_chain_features(self):
        ...
        print_and_log(self.log_file, f"Extracting chain features, NF project name: {project_name}")
        print_and_log(self.log_file, f"Project path: {project_path}")
        ...
        if self.para:  # run jobs with para, skip nextflow
            ...
            print_and_log(self.log_file, f"Calling parallel process with:\n{cmd}")
            rc = subprocess.call(cmd, shell=True)
    ...
    def __merge_chains_output(self):
        ...
        print_and_log(self.log_file, "Merging chain output...")
        merge_chains_output(
    ...
    def __classify_chains(self):
        ...
        print_and_log(self.log_file, "Classifying chains")
    ...
    def __get_proc_pseudogenes_track(self):
        ...
        print_and_log(self.log_file, "Creating processed pseudogenes track.")
    ...
    def __precompute_data_for_opt_cesar(self):
        ...
        print_and_log(self.log_file, "Computing memory requirements for optimized CESAR")
        ...
        project_path = os.path.join(self.nextflow_dir, project_name)
        print_and_log(self.log_file, f"Precomputing cesar regions, NF project name: {project_name}")
        print_and_log(self.log_file, f"Project path: {project_path}")

        if self.para:  # run jobs with para, skip nextflow
            cmd = f'para make {project_name} {precomp_mem_joblist} -q="shortmed"'
            print_and_log(self.log_file, f"Executing parallel step with {cmd}")
            rc = subprocess.call(cmd, shell=True)
        else:  # calling jobs with nextflow
        ...
        if self.output_opt_cesar_regions:
            ...
            print_and_log(self.log_file, f"Bed file containing precomputed regions is saved to: {save_to}")
            sys.exit(0)
    ...
    def __split_cesar_jobs(self):
        ...
        if self.fragmented_genome:
            print_and_log(self.log_file, "Detecting fragmented transcripts")
            ...
            for k, v in gene_fragments.items():
                ...
                f.write(f"{k}\t{v_str}\n")
            f.close()
            print_and_log(self.log_file, f"Detected {len(gene_fragments.keys())} fragmented transcripts")
            print_and_log(self.log_file, f"Fragments data saved to {fragm_dict_file}")
        else:
            # no fragment file: ok
            print_and_log(self.log_file, "Skip fragmented genes detection")
    ...
    def __monitor_jobs(self, processes, project_paths, die_if_sc_1=False):
        ...
        while True:  # Run until all jobs are done (or crashed)
            ...
            for p in processes:
                ...
                if running:
                    all_done = False
            if all_done:
                print_and_log(self.log_file, "### CESAR jobs done ###")
                break
            else:
                print_and_log(self.log_file,
                              (f"Status check iteration {iter_num} "
                               f"waiting {ITER_DURATION * iter_num} seconds."
                               )
                )
        ...
        if not self.keep_nf_logs:
            # remove nextflow intermediate files
            print_and_log(self.log_file, "Removing nextflow temp files")
    ...
    def __run_cesar_jobs(self):
        ...
        print_and_log(self.log_file, f"Pushing {len(buckets)} CESAR job lists")
        ...
        for b in buckets:
            ...
            if b != 0:  # extract jobs related to this bucket (if it's not 0)
                ...
                if len(bucket_tasks) == 0:
                    print_and_log(self.log_file, f"There are no jobs in the {b}Gb bucket")
                    continue
        ...
        for p_name in project_names:
            ...
            print_and_log(self.log_file, f"para time output for {p_name}:")
            print_and_log(self.log_file, stdout)
            print_and_log(self.log_file, stderr)
    ...
    def __check_cesar_completeness(self):
        ...
        if len(rejected_logs_filenames) == 0:
            # nothing crashed
            print_and_log(self.log_file, "No CESAR jobs crashed")
            return
        ...
        print_and_log(self.log_file, f"{crashed_jobs_num} CESAR jobs crashed, trying to run again...")
        ...
        for bucket, jobs in bucket_to_jobs.items():
            print_and_log(self.log_file, f"Pushing {len(jobs)} jobs into {bucket} GB queue")
        ...
        if len(crashed_twice) == 0:
            print_and_log(self.log_file, "All CESAR jobs re-ran succesfully!")
            return
    ...
    def __merge_cesar_output(self):
        ...
        print_and_log(self.log_file, "Merging CESAR output to make fasta and bed files.")
        ...
        if len(all_ok) == 0:
            ...
            print_and_log(self.log_file, "CESAR results merged")
            ...
            print_and_log(self.log_file, warning_message)
    ...
    def __gene_loss_summary(self):
        ...
        print_and_log(self.log_file, "Calling gene loss summary")
    ...
    def __orthology_type_map(self):
        ...
        print_and_log(self.log_file, "Calling orthology types mapping step...")
    ...
    def __check_crashed_cesar_jobs(self):
        ...
        print_and_log(self.log_file, f"{len(self.crashed_cesar_jobs)} CESAR wrapper commands failed")
    ...
    def __get_version(self):
        ...
        print_and_log(self.log_file, version)
        return version
